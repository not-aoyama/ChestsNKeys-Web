...
function displayChestUnlocked(chestNumber) {
    ...
    _jquery(chestID).attr("title", "Chest " + chestNumber + " (Unlocked)");
    // main-game.css gives all "clickable"-class objects a pointer cursor to show they can be clicked.
    _jquery(chestID).attr("class", "clickable");
    // Now that the chest is unlocked, clicking it should send a check.
    _jquery(chestID).click(()=>{
        (0, _loginJs.client).check(LOCATION_ID_PREFIX + chestNumber);
    });
    // Add the SVG icon.
    _jquery(chestID).append((0, _unlockedChestSvgDefault.default));
}
...
},{"jquery":"hgMhh","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./login":"ly455"}]},["aX1Mp","8WNDW"], "8WNDW", "parcelRequire04ca", {})
